...
import { ckOfEntry } from "../routing.js";
import { BranchPrHealthView } from "./BranchPrHealthView.js";
import { WorktreesLens } from "./WorktreesLens.js";
...
export function BranchPrHealth({ companyId }: { companyId: string | null }) {
  ...
  // lands directly on the Worktrees lens.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}>
      ...
      {lens === "branches" ? (
        ...
        <ConnectedWorktreesLens companyId={companyId} now={now} isMobile={isMobile} />
      )}
    </div>
  );
}
...
function ConnectedWorktreesLens({ companyId, now, isMobile }: { companyId: string | null; now: number; isMobile: boolean }) {
  ...
  return <WorktreesLens board={board} now={now} isMobile={isMobile} onOpenDoc={onOpenDoc} />;
}
