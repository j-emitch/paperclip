    ...
    return collectPerRepo(TEACHING_SOURCE_ID, ctx, async (repo, c): Promise<RepoReadResult> => {
      ...
      for (const file of files) {
        ...
        if (kind === "synthesis") {
          ...
          signals.push(baseSignal(repo.repo, file, c.hash(`${rel}:${file.mtime}`), null, syntheticMeta(rel), null));
          continue;
        }
        ...
        if (kind === "inbox") {
          signals.push(baseSignal(repo.repo, file, sha, null, inboxMeta(text), null));
        } else {
          const { title, meta, createdBy } = unitMetaFrom(rel, text);
          signals.push(baseSignal(repo.repo, file, sha, title, meta, createdBy));
        }
      }
...
function unitMetaFrom(
  rel: string,
  text: string,
): { title: string | null; meta: TeachingArtifactMeta; createdBy: string | null } {
  ...
  return { title, meta, createdBy: fm.created_by ?? null };
}
